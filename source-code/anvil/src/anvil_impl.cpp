...
namespace anvil {
    ...
    // sha256_file — only function actually used; sha256_hex removed to avoid unused-function warning
    ...
    public:
        ...
        Result<Manifest> build_manifest() override
        {
            ...
            audit("manifest-build", cfg_.paths.store_dir,
                  std::to_string(m.entries.size()) + " entries, gen-"
                ...
                if (me.is_symlink) {
                    ...
                    if (actual != me.link_target) {
                        report.violations.push_back({ ViolationKind::LinkChanged, me.path,
                            "expected \xe2\x86\x92 " + me.link_target
                            + "  actual \xe2\x86\x92 " + actual });
                    } else {
                        ++report.ok;
                    }
                } else {
                    ...
                    if (sz != me.size_bytes) {
                        report.violations.push_back({ ViolationKind::Modified, me.path,
                            "size " + std::to_string(me.size_bytes)
                            + " \xe2\x86\x92 " + std::to_string(sz) });
                        continue;
                    }
        ...
        Error audit(std::string_view action,
                    ...
                    Error load_state() override
                    {
                        ...
                        if (!f) {
                            return Error::make(ErrorCode::IoError, "Cannot read state file");
                        }
                        std::string json((std::istreambuf_iterator<char>(f)),
                                         std::istreambuf_iterator<char>());
