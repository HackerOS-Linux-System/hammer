...
//  Filesystem policy
//
//  These are the paths that anvil locks read-only.
//  /home and /var are explicitly excluded — they must stay writable.
// ─────────────────────────────────────────────────────────────

static const std::vector<std::string> RO_PATHS = {
    "/",
    "/usr",
    "/hammer",
    "/etc",
...
// These paths are NEVER locked — always remain read-write.
static const std::vector<std::string> RW_ALWAYS = {
    "/var",
    "/home",
    "/tmp",
    "/run",
...
static bool is_always_rw(const std::string& path) {
    for (const auto& rw : RW_ALWAYS) {
        if (path == rw || path.rfind(rw + "/", 0) == 0) {
            return true;
        }
    }
    return false;
...
static std::string json_get_str(const std::string& j, std::string_view key) {
    ...
    while (end != std::string::npos && j[end-1]=='\\')
        end = j.find('"', end+1);
    if (end == std::string::npos) return {};
    return j.substr(pos+1, end-pos-1);
}

static bool json_get_bool(const std::string& j, std::string_view key, bool def=false) {
    std::string needle = "\""; needle += key; needle += "\"";
    auto pos = j.find(needle);
...
static MountState detect_mount_state_one(const std::string& path) {
...
// Returns the mount state of the path that matters most for the
// "is the system locked?" question — we use "/" as primary.
static MountState detect_system_mount_state() {
    // Check root first
    auto root_state = detect_mount_state_one("/");
    if (root_state != MountState::Unknown) return root_state;
    // Fallback: check /hammer
    return detect_mount_state_one("/hammer");
}
...
static bool do_remount(const std::string& path, bool read_only) {
    ...
    return ::mount(nullptr, path.c_str(), nullptr, flags, nullptr) == 0;
#else
...
// Remount all RO_PATHS that are not in RW_ALWAYS
static std::vector<std::string> remount_all(bool read_only) {
    std::vector<std::string> done;
    for (const auto& p : RO_PATHS) {
        if (is_always_rw(p)) continue;
        if (!fs::exists(p))  continue;
        if (do_remount(p, read_only)) {
            done.push_back(p);
        }
    }
    return done;
...
public:
    ...
    // ── Lock ─────────────────────────────────────────────────
    //
    //  Lock makes / (and all other RO_PATHS) read-only.
    //  /var and /home are NEVER touched — always stay writable.
    ...
    Error lock() override {
        ...
        auto current = detect_system_mount_state();
        if (current == MountState::ReadOnly) {
            state_.locked    = true;
        ...
        std::string detail = locked_paths.empty()
            ? "state-only lock"
            : "remounted ro: " + join(locked_paths, ", ");
        audit("lock", "/", detail);
        return Error::ok_result();
    }
    ...
    // ── Unlock ───────────────────────────────────────────────
    //
    //  Unlock is temporary — hammer calls unlock before a
    //  transaction and lock after. /var and /home are untouched.
    ...
    Error unlock() override {
        ...
        auto current = detect_system_mount_state();
        if (current == MountState::ReadWrite) {
            state_.locked      = false;
        ...
        if (!cfg_.dry_run) {
            unlocked_paths = remount_all(false);
        }
        ...
        std::string detail = unlocked_paths.empty()
            ? "state-only unlock"
            : "remounted rw: " + join(unlocked_paths, ", ");
        audit("unlock", "/", detail);
        return Error::ok_result();
    }
    ...
    LockState get_lock_state() const override {
        LockState ls   = state_;
        ls.mount       = detect_system_mount_state();
    ...
    Result<Manifest> load_manifest() const override {
        ...
        if (!f) return Result<Manifest>::fail(
            Error::make(ErrorCode::IoError, "Cannot read manifest"));
        ...
        {
            auto s = json_get_str(json, "generation");
            m.generation = s.empty() ? 0u : uint32_t(std::stoul(s));
        }
            ...
            if (me.is_symlink) {
                ...
                if (!fs::exists(fs::symlink_status(me.path))) {
                    report.violations.push_back(
                        {ViolationKind::Missing, me.path, "symlink missing"});
                    continue;
                }
                ...
                if (actual != me.link_target) {
                    report.violations.push_back({ViolationKind::LinkChanged, me.path,
                        "expected \xe2\x86\x92 "+me.link_target+"  actual \xe2\x86\x92 "+actual});
                } else { ++report.ok; }
            } else {
                if (!fs::exists(me.path)) {
                    report.violations.push_back(
                        {ViolationKind::Missing, me.path, "file missing"});
                    continue;
                }
                std::error_code ec;
                ...
                if (sz != me.size_bytes) {
                    report.violations.push_back({ViolationKind::Modified, me.path,
                        "size "+std::to_string(me.size_bytes)+" \xe2\x86\x92 "+std::to_string(sz)});
                    continue;
                }
                ...
                if (actual != me.sha256) {
                    report.violations.push_back({ViolationKind::Modified, me.path,
                        ...
                        "\xe2\x80\xa6\n  actual   "+actual.substr(0,16)+"\xe2\x80\xa6"});
                } else { ++report.ok; }
            }
        }
    ...
    Result<VerifyReport> verify_generation(uint32_t gen_number) override {
        auto prof = fs::path(cfg_.paths.profiles_dir)
                    / ("gen-"+std::to_string(gen_number));
        if (!fs::exists(prof)) {
        ...
        std::error_code ec;
        for (auto& entry : fs::recursive_directory_iterator(
                prof, fs::directory_options::skip_permission_denied, ec)) {
            if (ec) { ec.clear(); continue; }
            ++report.checked;
            if (fs::is_symlink(entry.symlink_status())) {
                ...
                if (!fs::exists(target, ec)) {
                    report.violations.push_back({
                        ...
                        entry.path().string(),
                        "dangling symlink \xe2\x86\x92 "+target.string()
                    });
                } else { ++report.ok; }
            } else if (fs::is_regular_file(entry.status())) {
                ++report.ok;
    ...
    std::vector<ProtectedPath> list_protected() const override { return protected_; }

    Error add_protected(std::string_view path, std::string_view added_by) override {
        for (const auto& p : protected_) {
            if (p.path == path) return Error::ok_result();
        }
    ...
    Error remove_protected(std::string_view path) override {
        ...
        protected_.erase(
            std::remove_if(protected_.begin(), protected_.end(),
        ...
        return Error::make(ErrorCode::NotFound,
                           "Protected path not found: "+std::string(path));
    }

    std::vector<std::string> check_protected() const override {
        std::vector<std::string> failed;
        for (const auto& pp : protected_) {
            if (!fs::exists(pp.path)) failed.push_back(pp.path+" [missing]");
        }
        return failed;
    }
    ...
    Result<std::vector<AuditEntry>> read_audit(uint32_t tail) const override {
        ...
        uint32_t start = lines.size() > tail
                         ? uint32_t(lines.size())-tail : 0;
        std::vector<AuditEntry> entries;
        for (uint32_t i = start; i < uint32_t(lines.size()); ++i) {
    ...
    Error save_state() override {
        auto ms_str = [](MountState m) -> std::string {
            switch (m) {
        ...
        std::ostringstream j;
        j << "{\n"
          << "  \"mount\":          " << json_escape(ms_str(state_.mount))  << ",\n"
...
private:
    ...
    uint32_t current_gen() const {
        ...
        return s.empty() ? 0u : uint32_t(std::stoul(s));
    }
...
public:
    ...
    Error start(TamperCallback cb) override {
        ...
        // Watch key system directories
        for (const auto& p : {
            paths_.store_dir, paths_.db_dir, paths_.profiles_dir,
            std::string("/usr/bin"), std::string("/usr/lib"),
            std::string("/etc"),
        }) {
            add_watch(p);
        }
...
const char* anvil_version() { return ANVIL_VERSION_STR; }
