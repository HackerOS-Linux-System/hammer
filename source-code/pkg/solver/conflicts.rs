                ...
                if let Some(inst) = db.get(&alt.name) {
                    ...
                    if violates {
                        out.push(ConflictInfo {
                            message:  format!("Package '{}' conflicts with installed '{}' {}", candidate.name, inst.name, inst.version),
        ...
        for group in parse_dep_field(b_str) {
            for alt in &group.alternatives {
                if let Some(inst) = db.get(&alt.name) {
                    ...
                    if breaks_it {
                        out.push(ConflictInfo {
                            message:  format!("Package '{}' breaks installed '{}' {}", candidate.name, inst.name, inst.version),
...
pub fn reverse_depends(removing: &[String], db: &InstalledDb) -> Vec<String> {
    let remove_set: std::collections::HashSet<&str> = removing.iter().map(|s| s.as_str()).collect();
    ...
    for inst in &all {
        ...
        .any(|group| group.alternatives.iter().any(|a| remove_set.contains(a.name.as_str())));
