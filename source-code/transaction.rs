...
//
//  Stages:
//    1. Download  — per-package spinner + overall bar (yarn-style)
//    2. Unpack    — per-package spinner
//    3. Compose   — single spinner for profile composition
//    4. Stage / apply
//    5. Update DB
...
pub async fn execute_transaction(ctx: TransactionContext<'_>, note: &str) -> Result<u32> {
    ...
    let effective_store_dir = ctx.store_override.as_deref().unwrap_or_else(|| {
        std::path::Path::new(crate::store::STORE_DIR)
    });

    // ── 1. Download ───────────────────────────────────────────

    let all_pkgs: Vec<Package> = plan
    .to_install
    .iter()
    ...
    let dl_results = if !all_pkgs.is_empty() {
        ...
        println!(
            "  {}  {}",
            "⬡".bright_cyan().bold(),
                 format!("Fetching {} package{}…", all_pkgs.len(), if all_pkgs.len() == 1 { "" } else { "s" }).bold()
        );
    ...
    for dl in &dl_results {
        ...
        let entry =
        Store::install_deb_to(&dl.package, &deb, effective_store_dir)?;
    ...
    // Remove postinst scripts for removed packages
    for name in &plan.to_remove {
        remove_postinst(name);
    }
    ...
    let current_number = gens_db.current;
    ...
    let mut new_packages: Vec<StoreEntry> = {
        ...
        for inst in all_installed {
            ...
            let being_upgraded =
            plan.to_upgrade.iter().any(|p| p.name == inst.name);
            if being_removed || being_upgraded {
                continue;
            }
            let store_path = effective_store_dir.join(format!(
                "{}-{}-{}",
                inst.name, inst.version, inst.store_hash
            ));
            if store_path.exists() {
                entries.push(StoreEntry {
                    name:    inst.name,
                    version: inst.version,
                    hash:    inst.store_hash,
                    path:    store_path,
                });
            }
        }
    ...
    let gen = profile::compose_profile(
        gen_number,
        &new_packages,
        Some(note.to_string()),
    )?;

    compose_pb.finish_with_message(
        format!(
            "{}  gen-{} ready ({} packages)",
                "✔".bright_green(),
                gen_number,
    ...
    let applied_live = ctx.user_mode
    || (analysis.can_live_patch && !store_entries.is_empty());
