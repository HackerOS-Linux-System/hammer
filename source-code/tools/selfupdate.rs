...
fn parse_version_hacker(content: &str) -> Option<String> {
    // Extract the version number between [ and ]
...
// ─────────────────────────────────────────────────────────────
//  Check for update (returns Some(new_version) if update available)
// ─────────────────────────────────────────────────────────────
...
pub async fn check_for_update(client: &HttpClient) -> Result<Option<String>> {
    ...
    let local_ver = read_local_version().unwrap_or_else(|| env!("CARGO_PKG_VERSION").to_string());
...
pub async fn self_update(client: &HttpClient) -> Result<()> {
    ...
    println!(
        ...
        "⬡".bright_cyan().bold(),
        local_ver.dimmed(),
        new_version.bright_cyan().bold()
    );

    // 2. Download all three binaries into temp files BEFORE replacing anything
    ...
    let targets = vec![
        UpdateTarget {
            name:     "hammer",
            url:      format!("{}/{}/hammer", RELEASE_BASE, tag),
            dest:     HAMMER_BIN,
            required: true,
        },
        UpdateTarget {
            name:     "anvil",
            url:      format!("{}/{}/anvil", RELEASE_BASE, tag),
            dest:     ANVIL_BIN,
            required: false, // optional — anvil may not be in every release
        },
        UpdateTarget {
            name:     "hammer-store",
            url:      format!("{}/{}/hammer-store", RELEASE_BASE, tag),
            dest:     STORE_BIN,
            required: false,
        },
    ];

    // Download phase — download ALL before replacing ANY (atomic)
    let mut downloaded: Vec<(PathBuf, &str, bool)> = Vec::new();
    ...
    for target in &targets {
        ...
        match client.get_bytes(&target.url).await {
            Ok(bytes) => {
                ...
                downloaded.push((tmp, target.dest, true));
                println!("  {} {} downloaded ({} bytes)", "✔".green(), target.name, bytes.len());
            }
            Err(e) => {
                if target.required {
                    // Clean up already-downloaded temps
    ...
    // 3. Atomic replacement phase — rename temp → dest for each binary
    //    This is the "point of no return" — if rename fails partway through
    //    the system still has the old binaries (rename is atomic on Linux
    //    when source and dest are on the same filesystem).
    for (tmp_path, dest_str, _) in &downloaded {
    ...
    let version_tmp = format!("{}.tmp", VERSION_FILE);
    std::fs::write(&version_tmp, &version_content)?;
    std::fs::rename(&version_tmp, VERSION_FILE)?;
    ...
    println!(
        "  {}  Restart hammer for the new version to take effect.",
        "·".dimmed()
    );
    Ok(())
}
